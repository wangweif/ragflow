...
def create(tenant_id):
    ...
    if not invite_users:
        return get_data_error_result(message="用户未找到.")
    ...
    if user_tenants:
        ...
        if user_tenant_role == UserTenantRole.NORMAL:
            return get_data_error_result(message=f"{invite_user_email} 已在部门中.")
        if user_tenant_role == UserTenantRole.OWNER:
            return get_data_error_result(message=f"{invite_user_email} 是部门所有者.")
        return get_data_error_result(message=f"{invite_user_email} 在队伍中, 但角色: {user_tenant_role} 无效.")
